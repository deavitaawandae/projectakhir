                ...
                <div 
                            ...
                            <ul>
                                ...
                                <li>
                                    <a href="{{ route('show-recipe', ['id'=>$sr['id']]) }}" class="flex justify-between bg-white px-6 py-3 rounded-b-md transition hover:bg-gray-200 ease-out duration-200">
                                        <span>{{$sr['title']}}</span>
                                        <span class="text-xs">Resep Oleh {{User::where('id', '=', $sr['user_id'])->value('name')}}</span>
                                    </a>
                                </li>
                                @endforeach
                            </ul>
                    ...
                    @elseif($search != null && $searchResults->count() <= 0)
                        ...
                        x-show.transition.opacity="isOpen">
                            <ul>
                                <li>
                                    <div class="block bg-white px-6 py-3 rounded-b-md">Tidak Ditemukan Resep untuk "{{$search}}"</div>
                                </li>
                            </ul>
                        </div>
                    @endif
